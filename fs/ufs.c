...
int main(void) {
	char *disk_name = "icix.initrd";
	ufs *nfs = ufs_init(disk_name);
	ufs_creat(nfs, 0, UFS_DIRECTORY, "dir");
	ufs_creat(nfs, 0, UFS_REGULAR_FILE, "file1.txt");
	ufs_creat(nfs, 1, UFS_REGULAR_FILE, "file2.txt");
	const char *t1 = "Example contents of /file1.txt, stored on initrd";
	const char *t2 = "Example contents of /dir/file2.txt, stored on initrd";

	ufs_write(nfs, 2, t1, 0, sizeof(char) * strlen(t1));
	ufs_write(nfs, 3, t2, 0, sizeof(char) * strlen(t2));
